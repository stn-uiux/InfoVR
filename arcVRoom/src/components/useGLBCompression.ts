...
export async function compressGLBBuffer(
  ...
  name: string = 'export.glb',
  onProgress?: (step: string, progress: number) => void
): Promise<Uint8Array> {
  onProgress?.('Draco 압축 엔진 초기화 중...', 10);
  const io = new WebIO().registerExtensions(KHRONOS_EXTENSIONS);
  ...
  onProgress?.('3D 데이터 파싱 및 로딩 중...', 25);
  const isGLTF = name.toLowerCase().endsWith('.gltf');
  const doc = isGLTF
    ? await io.readJSON({
        json: JSON.parse(new TextDecoder().decode(originalBuffer)),
        resources: {}
      })
    : await io.readBinary(originalBuffer);

  const transforms = [];

  onProgress?.('기하학 버텍스 병합 및 최적화 중...', 45);
  if (options.flattenEnabled) transforms.push(flatten());
  if (options.instanceEnabled) transforms.push(instance());
  if (options.joinEnabled) transforms.push(join());
  if (options.weldEnabled) transforms.push(weld());
  ...
  const size = parseInt(options.textureSize);
  if (!isNaN(size)) {
    transforms.push(async (gltfDoc: Document) => {
      const textures = gltfDoc.getRoot().listTextures();
      const total = textures.length;
      let index = 0;
      for (const texture of textures) {
        index++;
        const percent = 65 + Math.round((index / total) * 15);
        onProgress?.(`고해상도 텍스처 WebP 압축 중 (${index}/${total})...`, percent);
        const mimeType = texture.getMimeType();
        const image = texture.getImage();
        if (!image) continue;
        ...
        const targetMime = options.textureFormat === 'webp' ? 'image/webp' :
          options.textureFormat === 'jpeg' ? 'image/jpeg' : 'image/png';
        ...
        if (resultBlob) {
          ...
          texture.setImage(new Uint8Array(arrayBuffer));
          texture.setMimeType(targetMime);
          texture.setURI(texture.getURI().replace(/\.[a-zA-Z0-9]+$/, `.${options.textureFormat}`));
        }
        URL.revokeObjectURL(img.src);
      }
    });
  ...
  onProgress?.('Draco 3D 데이터 최종 압축 중...', 80);
  await doc.transform(...transforms);

  onProgress?.('최종 GLB 바이너리 데이터 인코딩 중...', 95);
