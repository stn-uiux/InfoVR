...
import { type AppState, type FurnitureItem, type FurnitureType, type TextureConfig } from './types';
import { Scene } from './components/Scene';
import { UI } from './components/UI';
import { accentRgba, syncThemeColors } from './theme';
import { compressGLBBuffer } from './components/useGLBCompression';
...
export default function App() {
  ...
  const [exportProgress, setExportProgress] = useState<{ active: boolean, type: string, progress: number, details?: string } | null>(null);
  ...
  const exportScene = (mode: 'all' | 'objects' | 'lights' | 'json' = 'json') => {
    setExportProgress({
      active: true,
      type: mode === 'json' ? 'JSON' : (state.language === 'ko' ? 'GLB 파일 추출' : 'GLB Export'),
      progress: 0
    });
    ...
    exporter.parse(
      ...
      async (result) => {
        if (result instanceof ArrayBuffer) {
          setExportProgress({ active: true, type: state.language === 'ko' ? 'GLB 최적화/압축 중' : 'Optimizing & Compressing GLB', progress: 50 });
          try {
            const options = {
              textureFormat: 'webp',
              textureSize: '1024',
            ...
            const name = `export-${mode}-${new Date().toISOString().slice(0, 10)}.glb`;
            const uint8 = new Uint8Array(result);
            const optimizedBuffer = await compressGLBBuffer(uint8, options, name, (stepText, percent) => {
              setExportProgress({
                active: true,
                type: state.language === 'ko' ? 'GLB 최적화/압축' : 'GLB Optimization',
                progress: percent,
                details: stepText
              });
            });

            setExportProgress({ active: true, type: state.language === 'ko' ? 'GLB 내보내는 중' : 'Exporting GLB', progress: 95 });
            ...
            console.error('GLB Compression failed, exporting original:', compressErr);
            const blob = new Blob([result], { type: 'model/gltf-binary' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = `export-${mode}-${new Date().toISOString().slice(0, 10)}.glb`;
            link.click();
          } finally {
            setExportProgress(null);
          }
    ...
    if (batch.length > 0) {
      ...
      batch.forEach(item => {
        ...
        const newItem: FurnitureItem = {
          ...
          showBlackTop: (isWall || isStrokeWall)
        };
            ...
            >
              <div className="w-16 h-16 bg-teal-500/10 rounded-full flex items-center justify-center mb-6 relative">
                ...
                <motion.div
                  ...
                  animate={{ pathLength: smoothProgress / 100 }}
                  transition={{ duration: 0.5 }}
                  style={{ rotate: -90, position: 'absolute', inset: 0 }}
                />
                <Loader2 className="w-8 h-8 text-teal-500 animate-spin" />
              </div>
              <h3 className="text-white font-bold uppercase tracking-widest mb-2 text-sm">
                {state.language === 'ko' ? `${exportProgress.type} 중` : `${exportProgress.type}...`}
              </h3>
              <p className="text-teal-400 text-[11px] font-semibold tracking-tight h-5 truncate w-full">
                {exportProgress.details || (state.language === 'ko' ? '잠시만 기다려 주세요...' : 'Please wait a moment...')}
              </p>

              <div className="mt-8 w-full">
                <div className="flex justify-between items-center mb-1.5 text-[10px] font-bold text-teal-400/60 uppercase tracking-widest">
                  <span>{state.language === 'ko' ? '진행률' : 'Progress'}</span>
                  <span className="text-teal-400 font-mono text-[11px]">{smoothProgress}%</span>
                </div>
