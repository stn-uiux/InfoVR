...
const PortBox: React.FC<PortBoxProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`sentinel-port ${
        editMode ? 'sentinel-port--edit' : 'sentinel-port--view'
      } ${
        isSelected ? 'sentinel-port--selected' 
        : isActive ? 'sentinel-port--active' 
        : ''
      }`}
      style={{
        left: `${left}px`,
    ...
    >
      ...
      {port.svgType && (
        <img 
          ...
          alt={port.svgType}
          className="sentinel-port__svg-icon"
        />
      )}
      ...
      {port.svgPath && (
        <svg 
          className="sentinel-port__svg-path" 
      ...
      {(!editMode || (editMode && (isActive || isSelected))) && (
        <div className="sentinel-port__label">
          {port.portNumber}
        </div>
      )}
      ...
      {editMode && (
        <>
          ...
          {!isMultiSelect && (isPrimarySelection || (isActive && isSelected && !isPrimarySelection)) && (
            <div 
              className="sentinel-port__controls"
              style={{
            ...
            >
              {/* Name Input */}
              <div className="sentinel-port__field sentinel-port__field--tag">
                <span className="sentinel-port__field-label">Tag</span>
                <input 
                  type="text" 
                  value={port.portName || ''} 
                  ...
                  onFocus={saveHistory}
                  placeholder="tag"
                  className="sentinel-port__input"
                />
              </div>

              {/* Number Input */}
              <div className="sentinel-port__field sentinel-port__field--seq">
                <span className="sentinel-port__field-label">Seq</span>
                <input 
                  type="text" 
                  ...
                  onFocus={saveHistory}
                  placeholder="1"
                  className="sentinel-port__input sentinel-port__input--seq"
                />
              </div>

              <div className="sentinel-port__divider" />

              {/* SVG Type Selector */}
              <div className="sentinel-port__field sentinel-port__field--icon">
                <span className="sentinel-port__field-label">Icon</span>
                <select
                  value={port.svgType || ''}
                  onChange={(e) => {
                    saveHistory();
                  ...
                  onFocus={saveHistory}
                  className="sentinel-port__select"
                >
              ...
              {/* Dimensions */}
              <div className="sentinel-port__dims">
                <div className="sentinel-dims__col">
                  <span className="sentinel-dims__label">W</span>
                ...
                <div className="sentinel-dims__col">
                  <span className="sentinel-dims__label">H</span>
                  <div className="sentinel-dims__stepper">
                    <button 
              ...
              {/* Copy Buttons */}
              <div className="sentinel-port__copy-group">
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    window.dispatchEvent(new KeyboardEvent('keydown', { key: 'd', ctrlKey: true }));
                  }}
                  className="sentinel-port__copy-btn"
              ...
              <button 
                ...
                className="sentinel-port__delete-btn"
              >
                <X />
              </button>
            </div>
          ...
          {isSelected && (
            <div 
              className="sentinel-port__resize-wrap"
              onMouseDown={(e) => {
                e.stopPropagation();
                onResizeStart(idx, e);
              }}
            >
              <div className="sentinel-port__resize-handle">
                <Maximize2 />
              </div>
            </div>
          )}
