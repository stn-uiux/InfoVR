...
const getValue = (row: SheetRow | undefined, ...syns: string[]) => {
...
const filterData = (data: SheetRow[], selectedFields: string[]) => {
      ...
      try {
        ...
        const racks = racksFlat.map((r) => {
            ...
            .map((d) => {
              const devicePorts = portsFlat
                ...
                .map((p) => ({
                  ...
                  errorLevel: (p.errorLevel as ErrorLevel) || undefined,
                  errorMessage: (p.errorMessage as string) || undefined,
                }));

              const cardVal = d.insertedCards || d.cards;
              const moduleVal = d.insertedModules || d.modules;
          ...
          return {
            ...
            mapId: (r.mapId as string) || migrateGroupNameToNodeId(String(r.groupName || "과천")),
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        >[];
        const devicesFlat = deviceSheet
          ? (XLSX.utils.sheet_to_json(deviceSheet) as SheetRow[])
          : [];
        const portsFlat = portSheet
          ? (XLSX.utils.sheet_to_json(portSheet) as SheetRow[])
          : [];
        ...
        const rackDevices = devicesFlat.map((d) => {
          ...
          const moduleVal = getValue(d, "insertedModules", "modules");
	          ...
	          return {
	            itemId: String(d.deviceId || generateUUID()),
	            title: String(d.title || ""),
	            type: (d.type || "Server") as DeviceType,
	            size: Number(d.size),
	            position: Number(d.position),
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const rows = XLSX.utils.sheet_to_json(sheet) as SheetRow[];
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (metaSheet) {
          const metaRows = XLSX.utils.sheet_to_json(metaSheet) as SheetRow[];
          ...
          if (typeRow) exportScopeType = typeRow.value as "ALL" | "NODE";
        ...
        const racksRaw = rackSN ? (XLSX.utils.sheet_to_json(workbook.Sheets[rackSN]) as SheetRow[]) : [];
        const devsRaw = devSN ? (XLSX.utils.sheet_to_json(workbook.Sheets[devSN]) as SheetRow[]) : [];
        const portsRaw = portSN ? (XLSX.utils.sheet_to_json(workbook.Sheets[portSN]) as SheetRow[]) : [];
        const regDevsRaw = workbook.Sheets[regDevSN] ? (XLSX.utils.sheet_to_json(workbook.Sheets[regDevSN]) as SheetRow[]) : [];
        ...
        if (groupsSheet) {
          const rows = XLSX.utils.sheet_to_json(groupsSheet) as SheetRow[];
          fileNodes = rows.map(r => ({
            ...
            type: (r.nodeType || "group") as NodeType,
            order: Number(r.sortOrder || r.order || 0)
          }));
        }

        const getRowInfo = (row: SheetRow) => {
        ...
        racksRaw.forEach((r) => {
            ...
            .map(d => {
              ...
              const dPorts = portsRaw
                ...
                .map(p => ({
                  ...
                  status: (getValue(p, "status") || "normal") as "normal" | "error",
              ...
              return {
                ...
                type: (getValue(d, "type") || "Server") as DeviceType,
          ...
          dataByNode[nodeTarget].racks.push({
            ...
            rackSize: Number(getValue(r, "rackSize") || 48) as 24 | 32 | 48,
            width: Number(getValue(r, "width") || RACK_WIDTH_STANDARD),
            position: [Number(getValue(r, "posX") || 0), Number(getValue(r, "posZ") || 0)],
            orientation: Number(getValue(r, "orientation") || 0) as Orientation,
            devices: rDevices as Device[]
          });
        });
        ...
        regDevsRaw.forEach((d) => {
          ...
          dataByNode[nodeTarget].registeredDevices.push({
            ...
            vendor: getValue(d, "vendor") as VendorName | undefined,
            // [MODULAR] 안전하게 복구
            insertedCards: safeParseJson(cardVal),
            insertedModules: safeParseJson(moduleVal),
          });
