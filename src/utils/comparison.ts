...
/**
 * Deeply compares two objects, specifically tailored for the layouts and nodes in the ArcVRack project.
...
export function layoutsEqual(a: unknown, b: unknown): boolean {
  ...
  if (Array.isArray(a)) {
    ...
    if (a.length > 0 && typeof a[0] === 'number') {
      return areArraysClose(a, b as number[]);
    }
    for (let i = 0; i < a.length; i++) {
  ...
  if (typeof a === 'object') {
    ...
    for (const key of keysA) {
      ...
      if (!layoutsEqual((a as Record<string, unknown>)[key], (b as Record<string, unknown>)[key])) return false;
    }
    return true;
  }
