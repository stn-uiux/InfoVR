...
export function isPortId(id: string): boolean {
  ...
  if (lower === "ports-layer" || lower === "port-layer") return false;

  // 1. 기존 port- 포함 패턴 (프리픽스 대응)
  if (lower.includes("port-")) return true;

  // 2. port1, port_1, port-1 패턴
  if (/^port[-_]?\d+/i.test(lower)) return true;

  // 3. p1, p-1, p_1 패턴
  if (/^p[-_]?\d+/i.test(lower)) return true;

  return false;
}

/**
 * SVG 포트 엘리먼트에서 고유한 portId를 결정하는 공통 유틸리티.
 *
 * 우선순위:
 * 1. data-port-number (카드 합성 시 생성된 realPortNumber)
 * 2. 유효한 포트 ID (port-sfp-1 등)
 * 3. portType + localPort 조합 (예: sfp-1, console-1)  ← 핵심 fallback
 * 4. localPort만 (portType이 없는 경우)
 */
export function resolvePortId(el: Element): string {
  const realPortNumber = el.getAttribute("data-port-number");
  if (realPortNumber) return realPortNumber;

  // id가 유효한 포트 ID인 경우 사용
  if (el.id && isPortId(el.id)) return el.id;

  // fallback: portType + localPort 조합
  const localPort = el.getAttribute("data-local-port");
  const portType = el.getAttribute("data-port-type") || el.getAttribute("data-porttype");
  if (localPort) {
    if (portType && portType !== "unknown" && portType !== "port") {
      return `${portType}-${localPort}`;
    }
    return localPort;
  }
